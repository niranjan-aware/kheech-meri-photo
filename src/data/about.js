export const aboutData = {
  story: {
    title: 'Our Story',
    content: 'Founded with a passion for capturing life\'s most precious moments, KheechMeriPhoto has become Mumbai\'s trusted name in photography. With over a decade of experience, our team combines artistic vision with technical excellence to create timeless memories.',
    mission: 'To capture authentic emotions and create stunning visual stories that our clients will cherish for a lifetime.',
    vision: 'To be India\'s most loved and trusted photography brand, known for creativity, quality, and customer satisfaction.',
  },
  stats: [
    { number: '1000+', label: 'Happy Couples' },
    { number: '5000+', label: 'Events Covered' },
    { number: '50K+', label: 'Photos Delivered' },
    { number: '10+', label: 'Years Experience' },
  ],
  team: {
    title: 'Meet Our Team',
    description: 'A group of passionate professionals dedicated to capturing your special moments.',
    image: 'https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=800',
  },
  values: [
    {
      title: 'Creativity',
      description: 'We bring innovative ideas and artistic vision to every project.',
      icon: '🎨',
    },
    {
      title: 'Quality',
      description: 'We never compromise on the quality of our work and equipment.',
      icon: '⭐',
    },
    {
      title: 'Passion',
      description: 'We love what we do and it shows in every photograph we create.',
      icon: '❤️',
    },
    {
      title: 'Trust',
      description: 'We build lasting relationships with our clients based on trust and reliability.',
      icon: '🤝',
    },
  ],
};